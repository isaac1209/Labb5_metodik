...
int main() {
    ...
    Book arr[]={{"In Search of Lost Time ", 3},{"Moby Dick", 8},{"War and Peace", 13},
   ...
   auto it= std::adjacent_find(book_vec.begin(),book_vec.end(),first());
    ...
    auto it_2=std::search(book_vec.begin(),book_vec.end(),arr, arr+6); //tror inte att den funkar som den skulle
    if(it_2== book_vec.end()){
    ...
    std::cout<<"-----------------------TASK 6----------------------------"<<"\n\n";
        ...
        auto it_3=std::accumulate(book_vec.begin(),book_vec.end(),double() ,My_bin_op(book_vec.size()));
    ...
    std::cout<<"-----------------------TASK 7----------------------------"<<"\n\n";;
        ...
        std::transform(book_vec.begin(), book_vec.end(),v2.begin(), UnOP()); // returns the specified elemts of a data typ from a class or struct
